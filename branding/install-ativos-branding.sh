...
# =========================================================================
# 1c. systemd-boot menu entry title
#
#     THE BUG: entries generated by `kernel-install` (systemd-boot's own
#     entry generator, run automatically by a pacman hook on every kernel
#     install/upgrade) bake in whatever PRETTY_NAME was in /etc/os-release
#     *at the time the entry was generated* — i.e. "Arch Linux", from
#     before this script ever ran (this is exactly what archinstall's
#     default bootloader produces). os-release now says AtivOS (written in
#     step 1 above), which fixes every *future* kernel update automatically,
#     but does nothing for entries that already exist. Regenerate now and
#     sweep existing entry files directly so the rename is visible on the
#     very next boot.
# =========================================================================
echo "==> Configuring systemd-boot menu entry title"
...
if [[ -n "$SDBOOT_ENTRIES_DIR" ]]; then
    ...
    for conf in "$SDBOOT_ENTRIES_DIR"/*.conf; do
        [[ -f "$conf" ]] || continue
        grep -q 'Arch Linux' "$conf" || continue
        cp "$conf" "${conf}.bak.$(date +%s)" 2>/dev/null || true
        sed -i 's/Arch Linux/AtivOS/g' "$conf"
        RENAMED=1
    done
    if [[ $RENAMED -eq 1 ]]; then
        echo "    Renamed entries in $SDBOOT_ENTRIES_DIR to \"AtivOS\""
    else
        echo "    Entries in $SDBOOT_ENTRIES_DIR already say \"AtivOS\" (or use a custom title)"
    fi
else
    echo "    No systemd-boot entries directory detected — skipping."
fi
